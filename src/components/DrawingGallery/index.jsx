...
const DrawingGallery = () => {
    ...
    function handleBtnPrev() {
        ...
        var drawing_data_length = state.drawing_data.length - 1;
        var newIndex = state.current_gallery_index - 1 ; 
        if(newIndex < 0) {
            newIndex = drawing_data_length;
        }
    ...
    function handleBtnNext() {
        ...
        var limit = state.drawing_data.length - 1;
        var newIndex = state.current_gallery_index + 1 ; 
    ...
    return (
        <div className="sliderWrap fixed top-0 bottom-0 left-0 right-0 z-50 bg-gray-700 flex align-middle justify-center w-full h-full ">
            ...
            <div className="fixed top-64 right-60">
                <p className="py-1 text-3xl">{state.drawing_data[state.current_gallery_index].title}</p>
                <p className="py-1 text-3xl">{state.drawing_data[state.current_gallery_index].materials}</p>
                <p className="py-1 text-3xl">{state.drawing_data[state.current_gallery_index].dimensions}</p>
                <p className="py-1 text-3xl">{state.drawing_data[state.current_gallery_index].created_date}</p>
            </div>
            <div className="fullScreenImage flex align-middle justify-center m-24 opacity-100">
                <img src={state.drawing_data[state.current_gallery_index].url} alt='' />
            </div>
        </div>
    )
}
